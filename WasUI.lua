...
local function CreateShortcutButton(displayName, isToggle, initialState, onToggleCallback, onClickCallback, rainbowKey)
    ...
    if savedPos then btnFrame.Position = savedPos
    else
        local index = 0
    ...
    local dragData = {
        dragging = false, startPos = nil, startMouse = nil, moved = false, threshold = 5,
        connectionChanged = nil, connectionEnded = nil, currentTouch = nil
    }
    ...
    local function onInputEnded(input, processed)
        ...
        elseif input.UserInputType == Enum.UserInputType.Touch and dragData.dragging and input == dragData.currentTouch then isValid = true end
        if isValid then
            if dragData.moved then SaveShortcutPosition(key, btnFrame.Position)
            else
                if isToggle then
...
function ToggleSwitch:New(name, parent, title, initialState, onToggle, featureName, rainbowName, iconName, configKey, tips)
    local self = Control:New(name, parent)
...
function ProgressBar:New(name, parent, title, min, max, defaultValue, callback, segments)
    local self = Control:New(name, parent)
    self.Min = min or 0
    self.Max = max or 100
    self.Value = math.clamp(defaultValue or self.Min, self.Min, self.Max)
...
function WasUI:CreateProgressBar(parent, title, min, max, defaultValue, callback, segments)
...
function WasUI:ShowConfirmDialog(options, callback)
    ...
    confirmButton.MouseButton1Click:Connect(function()
        ...
        if showInput and inputBox then inputValue = inputBox.Text end
...
local function AnimateThemeChange(oldTheme, newTheme)
    ...
    for _, obj in ipairs(WasUI.Objects) do
        ...
        elseif obj.Type == "Dropdown" then
            ...
            if titleLabel and titleLabel:IsA("TextLabel") then Tween(titleLabel, {TextColor3 = newTheme.Text}, duration) end
            if dropdownButton and dropdownButton:IsA("TextButton") then Tween(dropdownButton, {BackgroundColor3 = newTheme.Input, TextColor3 = newTheme.Text}, duration) end
        ...
        elseif obj.Type == "Category" then
            local titleLabel = instance:FindFirstChild("TitleContainer"):FindFirstChild("Title"); local line = instance:FindFirstChild("Line")
...
function Panel:New(name, parent, size, position, backgroundUrl, snowEnabled, titleTag)
    ...
    function self:SetBackground(url)
        ...
        if url and url ~= "" then
            self.BackgroundImage = CreateInstance("ImageLabel", {Name = "Background", Size = UDim2.new(1, 0, 1, 0), Position = UDim2.new(0, 0, 0, 0), BackgroundTransparency = 1, Image = "", ImageTransparency = 0.2, ScaleType = Enum.ScaleType.Crop, ZIndex = 0, Parent = self.Instance})
            cp:PreloadAsync({url}); self.BackgroundImage.Image = url
    ...
    if backgroundUrl and backgroundUrl ~= "" then self:SetBackground(backgroundUrl) end
    AddRipple(self.Instance)
    ...
    function self:SetRainbowMode(mode)
        if mode == "整体" or mode == "流动" then
            ...
            if mode == "整体" then
                ...
                self.BorderStroke.Enabled = true; self.GlowStroke1.Enabled = true; self.GlowStroke2.Enabled = true; self.GlowStroke3.Enabled = true; self.GlowStroke4.Enabled = true; self.GlowStroke5.Enabled = true
                flowGradient.Enabled = false
            else
                self.BorderFlow.BackgroundTransparency = 0
                self.BorderStroke.Enabled = false; self.GlowStroke1.Enabled = false; self.GlowStroke2.Enabled = false; self.GlowStroke3.Enabled = false; self.GlowStroke4.Enabled = false; self.GlowStroke5.Enabled = false
    ...
    local function startFlowAnimation()
        ...
        self.BorderConnection = runS.Heartbeat:Connect(function(deltaTime)
            if self.RainbowMode == "整体" then
                ...
                self.BorderStroke.Transparency = 1; self.GlowStroke1.Transparency = 1; self.GlowStroke2.Transparency = 1; self.GlowStroke3.Transparency = 1; self.GlowStroke4.Transparency = 1; self.GlowStroke5.Transparency = 1
            end
        end)
    end
    ...
    if type(titleTag) == "table" then
        if titleTag[1] then titleTagsList = titleTag
        else titleTagsList = {titleTag} end
    elseif titleTag then titleTagsList = {titleTag} end
    if #titleTagsList > 0 then
        local titleContainer = CreateInstance("Frame", {Name = "TitleContainer", Size = UDim2.new(1, -120, 1, 0), Position = UDim2.new(0, 60, 0, 0), BackgroundTransparency = 1, Parent = self.TitleBar, ZIndex = 2})
        self.TitleContainer = titleContainer
        CreateInstance("UIListLayout", {FillDirection = Enum.FillDirection.Horizontal, HorizontalAlignment = Enum.HorizontalAlignment.Left, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 6), SortOrder = Enum.SortOrder.LayoutOrder, Parent = titleContainer})
        ...
        for _, tag in ipairs(titleTagsList) do
            local tagContainer = CreateInstance("Frame", {Name = "TitleTagContainer", Size = UDim2.new(0, 0, 0, 18), BackgroundColor3 = tag.backgroundColor or WasUI.CurrentTheme.Accent, BackgroundTransparency = 0.2, BorderSizePixel = 0, Parent = titleContainer, ZIndex = 10})
            CreateInstance("UICorner", {CornerRadius = UDim.new(0, 4), Parent = tagContainer})
            local tagLabel = CreateInstance("TextLabel", {Name = "TagLabel", Size = UDim2.new(1, -6, 1, 0), Position = UDim2.new(0, 3, 0, 0), BackgroundTransparency = 1, Text = tag.text, TextColor3 = tag.textColor or WasUI.CurrentTheme.Text, Font = Enum.Font.GothamSemibold, TextSize = 11, TextXAlignment = Enum.TextXAlignment.Center, TextYAlignment = Enum.TextYAlignment.Center, Parent = tagContainer, ZIndex = 11})
            task.wait(); local textWidth = tagLabel.TextBounds.X; tagContainer.Size = UDim2.new(0, textWidth + 8, 0, 18); tagLabel.Size = UDim2.new(0, textWidth, 1, 0)
    ...
    self.DotContainer = CreateInstance("Frame", {Name = "DotContainer", Size = UDim2.new(0, 60, 1, 0), Position = UDim2.new(0, 0, 0, 0.8), BackgroundTransparency = 1, ZIndex = 3, Parent = self.TitleBar})
    self.DotAreaButton = CreateInstance("ImageButton", {Name = "DotAreaButton", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Image = "", AutoButtonColor = false, ZIndex = 6, Parent = self.DotContainer})
    self.CloseDot = CreateInstance("Frame", {Name = "Close", Size = UDim2.new(0, 10, 0, 10), Position = UDim2.new(0, 11.2, 0.5, -5.4), BackgroundColor3 = Color3.fromRGB(255, 95, 87), BackgroundTransparency = 0, BorderSizePixel = 0, ZIndex = 4, Parent = self.DotContainer})
    self.MinimizeDot = CreateInstance("Frame", {Name = "Minimize", Size = UDim2.new(0, 10, 0, 10), Position = UDim2.new(0, 26.2, 0.5, -5.4), BackgroundColor3 = Color3.fromRGB(255, 189, 46), BackgroundTransparency = 0, BorderSizePixel = 0, ZIndex = 4, Parent = self.DotContainer})
    self.MaximizeDot = CreateInstance("Frame", {Name = "Maximize", Size = UDim2.new(0, 10, 0, 10), Position = UDim2.new(0, 41.2, 0.5, -5.4), BackgroundColor3 = Color3.fromRGB(39, 201, 63), BackgroundTransparency = 0, BorderSizePixel = 0, ZIndex = 4, Parent = self.DotContainer})
    ...
    -- 省略其余 Panel 初始化代码（与之前基本一致，确保 F1 热键逻辑已在 CreateWindow 中启用）
    self:EnableHotkeyToggle(Enum.KeyCode.F1)  -- 确保默认F1生效
    return self
end
...
function WasUI:CreateButton(parent, text, onClick, size, iconName, tips)
    return Button:New("Button", parent, text, onClick, size, iconName, tips)
end

function WasUI:CreateToggle(parent, initialState, onToggle, featureName, rainbowName, configKey, tips)
    return self:CreateToggleWithTitle(parent, featureName or "", initialState, onToggle, featureName, rainbowName, nil, configKey, tips)
end

function WasUI:CreateToggleWithTitle(parent, title, initialState, onToggle, featureName, rainbowName, iconName, configKey, tips)
...
function WasUI:CreateParagraph(parent, options)
