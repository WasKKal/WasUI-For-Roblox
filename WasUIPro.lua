...
function Button:New(options, parent)
    ...
    if options.Tooltip then
        WasUI:CreateTooltip(self.Instance, options.Tooltip)
    end
    ...
    self.Instance.MouseButton1Up:Connect(function()
        if isPressed then
            ...
            Tween(self.Instance, {BackgroundColor3 = WasUI.CurrentTheme.Secondary}, 0.1)
            SpringTween(scale, {Scale = 1}, 0.25)
            if not isDragging and options.Callback then
                options.Callback()
            end
        end
        if dragConnection then dragConnection:Disconnect() end
        ...
        self.Instance.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.Touch then
                if isPressed then
                    ...
                    Tween(self.Instance, {BackgroundColor3 = WasUI.CurrentTheme.Secondary}, 0.1)
                    SpringTween(scale, {Scale = 1}, 0.25)
                    if not touchDragging and options.Callback then
                        options.Callback()
                    end
                end
...
function ToggleSwitch:New(options, parent)
    ...
    if options.Tooltip then
        WasUI:CreateTooltip(self.Container, options.Tooltip)
    end
    ...
    local function performToggle(newState)
        ...
        if options.ConfigKey and WasUI.ConfigManager then
            ...
            if config then
                config:Set(options.ConfigKey, self.Toggled)
                config:Save()
            end
        end
    end
    ...
    if options.ConfigKey and WasUI.ConfigManager then
        local config = WasUI.ConfigManager:GetConfig(WasUI.ConfigFolderName .. "_settings")
        if config then
            config:Bind(options.ConfigKey, self, function(state) performToggle(state) end)
        end
    end
...
function Label:New(options, parent)
    ...
    self.Instance = CreateInstance("TextLabel", {
        ...
        BackgroundTransparency = 1,
        Text = "",
        TextColor3 = options.TextColor or WasUI.CurrentTheme.Text,
        TextTransparency = 0,
        Font = Enum.Font.Gotham,
        TextSize = 12,
        TextWrapped = true,
...
function Category:New(options, parent)
    ...
    if panel and panel.SetCurrentCategory then
        panel:SetCurrentCategory(options.Title)
    end
...
function Dropdown:New(options, parent)
    ...
    self.Options = {}
    for _, v in ipairs(options.Values or {}) do
    ...
    if self.MultiSelect then
        ...
        elseif options.Value ~= nil then
            table.insert(self.SelectedValues, tostring(options.Value))
        end
    else
        if type(options.Value) == "table" then
            self.SelectedValue = tostring(options.Value[1] or "")
        elseif options.Value ~= nil then
            self.SelectedValue = tostring(options.Value)
        end
    end
            ...
            optionButton.MouseButton1Click:Connect(function()
                if self.MultiSelect then
                    ...
                    if options.ConfigKey and WasUI.ConfigManager then
                        local config = WasUI.ConfigManager:GetConfig(WasUI.ConfigFolderName .. "_settings")
                        if config then
                            config:Set(options.ConfigKey, self.SelectedValues)
                            config:Save()
                        end
                    end
                else
                    ...
                    if options.ConfigKey and WasUI.ConfigManager then
                        local config = WasUI.ConfigManager:GetConfig(WasUI.ConfigFolderName .. "_settings")
                        if config then
                            config:Set(options.ConfigKey, option)
                            config:Save()
                        end
                    end
                end
...
function Slider:New(options, parent)
    ...
    local function setValueImmediately(newValue)
        ...
        if options.ConfigKey and WasUI.ConfigManager then
            local config = WasUI.ConfigManager:GetConfig(WasUI.ConfigFolderName .. "_settings")
            if config then
                config:Set(options.ConfigKey, self.Value)
                config:Save()
            end
        end
    end
...
function TextInput:New(options, parent)
    ...
    self.TextBox:GetPropertyChangedSignal("Text"):Connect(function()
        ...
        if options.ConfigKey and WasUI.ConfigManager then
            local config = WasUI.ConfigManager:GetConfig(WasUI.ConfigFolderName .. "_settings")
            if config then
                config:Set(options.ConfigKey, self.TextBox.Text)
                config:Save()
            end
        end
    end)
...
function Panel:New(options, parent)
    ...
    self.Instance = CreateInstance("Frame", {
        Name = options.Title,
        Size = options.Size or UDim2.new(0, 380, 0, 350),
    ...
    local borderTime = 0
    self.RainbowMode = options.RainbowMode or WasUI.DefaultRainbowMode
    self.FlowRotation = 0
    ...
    self.MinimizedTextLabel = CreateInstance("TextLabel", {
        ...
        AnchorPoint = Vector2.new(0.5, 0.5),
        BackgroundTransparency = 1,
        Text = options.MinimizedText or "WasUI",
...
function WasUI:CreateWindow(options)
    ...
    function windowAPI:Tab(tabOptions)
        ...
        local tabAPI = {}
        local function addToTab(controlType, createFunc, options)
            options.Parent = tabFrame
            local control = createFunc(options)
            return control
        end
        tabAPI.Button = function(opts) return addToTab("Button", Button.New, opts) end
        tabAPI.Toggle = function(opts) return addToTab("Toggle", ToggleSwitch.New, opts) end
        tabAPI.Label = function(opts) return addToTab("Label", Label.New, opts) end
        tabAPI.Category = function(opts) return addToTab("Category", Category.New, opts) end
        tabAPI.Dropdown = function(opts) return addToTab("Dropdown", Dropdown.New, opts) end
        tabAPI.Slider = function(opts) return addToTab("Slider", Slider.New, opts) end
        tabAPI.TextInput = function(opts) return addToTab("TextInput", TextInput.New, opts) end
        tabAPI.ProgressBar = function(opts) return addToTab("ProgressBar", ProgressBar.New, opts) end
        ...
        tabAPI.AddSpacing = function(height)
            WasUI:AddSpacing(tabFrame, height)
        end
        return tabAPI
    end
    
    function windowAPI:Category(categoryOptions)
        local category = Category.New(categoryOptions, window.Instance)
        return category
    end
    ...
    function windowAPI:Notify(notifOptions)
        return WasUI:Notify(notifOptions)
    end
